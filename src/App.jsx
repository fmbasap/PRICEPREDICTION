...
const ASSETS = {
  FLR: { source: "coingecko", id: "flare-networks", label: "Flare", ticker: "FLR", accent: "#E8A33D", currency: "usd" },
  XRP: { source: "coingecko", id: "ripple", label: "XRP", ticker: "XRP", accent: "#4FD1C5", currency: "usd" },
  BTC: { source: "coingecko", id: "bitcoin", label: "Bitcoin", ticker: "BTC", accent: "#F7931A", currency: "usd" },
  SOL: { source: "coingecko", id: "solana", label: "Solana", ticker: "SOL", accent: "#9945FF", currency: "usd" },
  SAMSUNG: { source: "stock", symbol: "005930.KS", label: "삼성전자", ticker: "005930", accent: "#5B9BD5", currency: "krw" },
};
...
export default function CryptoTrendDashboard() {
  ...
  const fetchAsset = useCallback(async (key, tf) => {
    const meta = ASSETS[key];
    const tfConf = TIMEFRAMES[tf];

    if (meta.source === "stock") {
      const url = `/api/stock?symbol=${encodeURIComponent(meta.symbol)}&range=${tfConf.stockRange}&interval=${tfConf.stockInterval}`;
      const res = await fetch(url);
      const json = await res.json();
      if (!res.ok || json.error) throw new Error(json.error || `데이터를 불러오지 못했습니다 (${res.status})`);
      return json.prices;
    }
  ...
  const loadAll = useCallback(
    async (tf) => {
      ...
      try {
        ...
        const results = await Promise.all(keys.map((k) => fetchAsset(k, tf)));
        const next = {};
        keys.forEach((k, i) => {
          next[k] = results[i];
        });
        ...
        {analysis && (
          <>
            <section style={styles.hero}>
              ...
              <div style={styles.heroPrice}>{fmtPrice(analysis.currentPrice, meta.currency)}</div>
              ...
              <ResponsiveContainer width="100%" height={280}>
                <ComposedChart data={analysis.chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <YAxis
                    ...
                    domain={["auto", "auto"]}
                    tickFormatter={(v) =>
                      meta.currency === "krw" ? Math.round(v).toLocaleString("ko-KR") : v < 0.01 ? v.toFixed(5) : v.toFixed(3)
                    }
                    axisLine={false}
                    tickLine={false}
                    width={meta.currency === "krw" ? 62 : 54}
                  />
                  <Tooltip content={<CustomTooltip accent={meta.accent} currency={meta.currency} />} />
                  <ReferenceLine
...
function CustomTooltip({ active, payload, label, accent, currency }) {
    ...
    <div style={styles.tooltip}>
      ...
      {price == null && projection != null && (
        <div style={{ fontFamily: "IBM Plex Mono, monospace", fontSize: 12, color: "#EDEAE3" }}>
          추세선 {fmtPrice(projection, currency)}
        </div>
      )}
    </div>
  );
